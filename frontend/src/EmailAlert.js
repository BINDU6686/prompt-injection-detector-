    ...
    <div style={{ padding: 28, fontFamily: "'Segoe UI', sans-serif", background: "#0A0A0A", minHeight: "100vh" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 20 }}>
        ...
        <div style={{ background: "#111111", border: "1px solid #222", borderRadius: 12, padding: 20 }}>
          ...
          <input
            ...
            style={{ width: "100%", padding: "11px 14px", border: `1px solid ${emailSaved ? "#00C49A" : "#333"}`, borderRadius: 8, fontSize: 13, fontFamily: "monospace", outline: "none", color: "#FFFFFF", background: "#1A1A1A", marginBottom: 12 }}
          />
          ...
          {emailSaved && (
            <div style={{ marginTop: 12, padding: "10px 14px", background: "rgba(0,196,154,0.1)", border: "1px solid rgba(0,196,154,0.3)", borderRadius: 8 }}>
              <div style={{ fontSize: 11, color: "#00C49A", fontFamily: "monospace" }}>Alert emails will be sent to: {email}</div>
              <div style={{ fontSize: 10, color: "#555", marginTop: 4, fontFamily: "monospace" }}>Every injection attack detected will trigger an automatic alert.</div>
            </div>
          )}
        </div>
        ...
        <div style={{ background: "#111111", border: "1px solid #222", borderRadius: 12, padding: 20 }}>
          ...
          {[
            { n: "1", t: "User sends a message to the AI chatbot" },
            { n: "2", t: "Detection framework analyses the message" },
            { n: "3", t: "If injection detected — attack is blocked" },
            { n: "4", t: "Automatic email alert sent to security team" },
            { n: "5", t: "Alert includes prompt text, confidence score and timestamp" },
          ].map((s, i) => (
      ...
      <div style={{ background: "#111111", border: "1px solid #222", borderRadius: 12, padding: 20, marginBottom: 20 }}>
        ...
        <textarea
          ...
          placeholder="Type an injection attack to trigger an email alert..."
        ...
        <button onClick={testDetection} disabled={loading || !prompt.trim() || !emailSaved} style={{ width: "100%", padding: 12, background: emailSaved ? "linear-gradient(135deg,#C9A84C,#F5D76E)" : "#1A1A1A", color: emailSaved ? "#0A0A0A" : "#444", border: emailSaved ? "none" : "1px solid #333", borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: emailSaved ? "pointer" : "not-allowed", fontFamily: "monospace" }}>
          {loading ? "Testing..." : emailSaved ? "Test Detection and Send Alert" : "Save email first to test"}
        </button>
        {result && !result.error && (
          <div style={{ marginTop: 12, padding: 14, background: result.is_injection ? "rgba(255,68,68,0.1)" : "rgba(0,196,154,0.1)", border: `1px solid ${result.is_injection ? "rgba(255,68,68,0.3)" : "rgba(0,196,154,0.3)"}`, borderRadius: 8 }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: result.is_injection ? "#FF4444" : "#00C49A", marginBottom: 4, fontFamily: "monospace" }}>
              {result.is_injection ? "INJECTION DETECTED — Email Alert Sent!" : "SAFE PROMPT — No Alert Needed"}
            </div>
      ...
      <div style={{ background: "#111111", border: "1px solid #222", borderRadius: 12, overflow: "hidden" }}>
        <div style={{ padding: "13px 18px", borderBottom: "1px solid #222", fontSize: 13, fontWeight: 700, color: "#C9A84C", fontFamily: "monospace" }}>// Alert Log — Simulated Emails Sent</div>
        {alerts.length === 0 ? (
          ...
          alerts.map((alert, i) => (
            <div key={alert.id} style={{ padding: "14px 18px", borderBottom: i < alerts.length - 1 ? "1px solid #1A1A1A" : "none", display: "flex", alignItems: "center", gap: 16 }}>
              <div style={{ width: 40, height: 40, borderRadius: 8, background: "rgba(255,68,68,0.15)", border: "1px solid rgba(255,68,68,0.3)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
              ...
              <div style={{ textAlign: "right" }}>
                ...
                <div style={{ padding: "2px 8px", borderRadius: 10, fontSize: 10, background: "rgba(0,196,154,0.15)", color: "#00C49A", fontFamily: "monospace" }}>{alert.status}</div>
              </div>
            </div>
          ))
        )}
